...
def material_and_position_eval(board: chess.Board) -> int:
    ...
    for piece_type, value in PIECE_VALUES.items():
     score_white += value * len(board.pieces(piece_type, chess.WHITE))
     score_black += value * len(board.pieces(piece_type, chess.BLACK))
...
class AlphaBetaEngine:
    ...
    def _negamax(self, board: chess.Board, depth: int, alpha: int, beta: int, ply: int, null_allowed: bool) -> int:
        ...
        if in_check:
            depth += 1
        if board.gives_check(list(board.legal_moves)[0]):
         depth += 1
